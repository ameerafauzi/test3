	...
	window.oldSelectFunctions = function() {
			...
			if (variant) {
				...
				if ( variant.featured_image != null ) {
					...
					if ( typeof VIG !== 'undefined' ) {
						...
						if ( $('.box__product-gallery .site-box-content').hasClass('flickity-enabled') ) {
						...
						if ( window.CUBER.Product.productFlkty != undefined && variantImg.data('index') != window.CUBER.Product.productFlkty.selectedElement ) {
							window.CUBER.Product.productFlkty.select(variantImg.data('index'));
						} else {
		...
		if ( $('#add-to-cart').hasClass('style--minimal') ) {
			...
			$('.product-variant').each(function(){
				...
				var color = returnColorVariant($(this).find('label').text()) ? true : false,
			...
			if ( window.disableProductVariants ) {
		 		$('.product-variant').find('li').addClass('disabled');
			} 
	...
	window.CUBER = {
		...
		Product: {
			...
			mount: function() {	
				...
				this.$productGallery.append('<div class="gallery-index out-with-you"><span class="current">' + ( window.CuberProductImageIndex != undefined ? ( window.CuberProductImageIndex + 1 ) : 1 ) + '</span> / <span class="total">' + this.$productGalleryItem.length + '</span></div>');
				...
				if ( this.$productGallery.hasClass('scroll') ) {
					...
					$(window).on('scroll.product-gallery', (function() {
						...
						if ( ! this.$productCarousel.hasClass('flickity-enabled') ) {
							...
							this.$productGalleryItem.each((function(key, elm){
				...
				if ( this.$productGallery.hasClass('slider') ) {
					...
					this.$productGallery.find('.gallery-index').append('<span class="icon-go go-prev">' + $.themeAssets.arrowRight + '</span><span class="icon-go go-next">' + $.themeAssets.arrowRight + '</span>');

					this.$productGallery.find('.go-prev').on('click', (function(){
						this.productFlkty.previous();
					}).bind(this));

					this.$productGallery.find('.go-next').on('click', (function(){
				...
				if ( $('#product-zoom-in').length > 0 ) {
					...
					$('#product-zoom-in').on('click', (function(e){
						...
						$image.attr('src', $('.gallery-item').eq(parseInt(this.$productGalleryIndex.html())-1).find('img').data('srcset')['huge']);
				...
				if ( $('#add-to-cart').length > 0 && $('#add-to-cart').data('type') == 'overlay' && ! $('html').hasClass('ie9') ) {
		...
		Main: {
				...
				if ( this.$body.hasClass('template-index') ) {
					...
					if ( this.$body.hasClass('template-product') ) {
						...
						if ( $('#fix-me-header').css('content') == '"fix-gallery-item"' ) {
							$('.gallery-item').css('height', ( $(window).height() - this.$siteHeader.outerHeight() ) * 0.8 );
						} else {
							//$('.gallery-item').css('height', $(window).height() - this.$siteHeader.outerHeight());
						}

						$('#section-product').removeClass('sticky-because');
						if ( $('.site-box.box__product-gallery').css('content') != '"fix-me-also"' && $('.site-box.box__product-gallery').height() < $('.site-box.box__product-content').height() ) {
    		...
    		$('html.no-touchevents a[href]').each(function(){
	    		...
	    		$(this).on('click', function(e){
	    			...
	    			if ( $(this).attr('target') == '_blank' || $(this).hasClass('video-lightbox') || $(this).hasClass('lightbox') || $(this).hasClass('block-fade') || $(this).attr('href') == '' || $(this).attr('href').indexOf('#') >= 0 ) {
	    				//
	    			} else {
		...
		Contact: {
				...
				if ( $('#contact-map-holder').length > 0 ) {
					...
					if ( $('#contact-map').data('address') != '' ) {
						...
						if ( typeof google !== 'undefined' ) {
							...
							$.getScript('https://maps.googleapis.com/maps/api/js?v=3&key='+ $('#contact-map-holder').data('key')).done((function(){
								this._createMap();
							}).bind(this))
						}
		    ...
		    if ( mapLat != 0 || mapLong != 0 ) {
	          ...
	          title: address
			    ...
			    $('#contact-map-address a').attr('href', 'http://www.google.com/maps/place/' + mapLat + ',' + mapLong);
			          ...
			          if (status != google.maps.GeocoderStatus.ZERO_RESULTS) {
			          	...
			          	map.setCenter(results[0].geometry.location);
					          ...
					          title: address
							    ...
							    $('#contact-map-address a').attr('href', 'http://www.google.com/maps/place/' + results[0].geometry.location.lat() + ',' + results[0].geometry.location.lng());
							    var contentString = $('#contact-map-address').html();
					        var infowindow = new google.maps.InfoWindow({
					          content: contentString
					        });
		...
		SplitSlider: {
			...
			_mountFlickity: function() {
				...
				if ( $slider.find('.slider-meta').length <= 0 ) {
					...
					$slider.append('<div class="slider-meta hide lap--show"><div class="slider-index"><span class="current">1</span> / <span class="total">' + t + '</span></div><div class="slider-nav"><span class="go-prev">' + $.themeAssets.arrowRight + '</span><span class="go-next">' + $.themeAssets.arrowRight + '</span></div>' );
			...
			mount: function( flick ) {	
				...
				var $slider = $('.box__slideshow-split'),
						...
						i = Math.min( Math.ceil(currentScroll / $(window).height()), $slides.length-1 );
						j = i-1,
						t = $slides.length;
				...
				$slidesMedia.each(function(key){
					...
					if ( key > 0 ) {
						...
						if ( key < i ) {
							...
							$(this).css('clip', 'rect(0 ' + Math.ceil($(window).width() / 2) + 'px ' + ( Math.ceil($(window).scrollTop() - $(window).height() * j ) ) + 'px 0)');
						} else {
							$(this).css('clip', 'rect(0 ' + Math.ceil($(window).width() / 2) + 'px 1px 0)');
						}
				...
				$(window).on('scroll.split-slider', function(e){
					...
					if ( currentScroll < $(window).scrollTop() ) {
						...
						// next
						if ( $slides.eq(i+1).length > 0 && $(window).scrollTop() + $(window).height() >= $slides.eq(i+1).offset().top) {

							if ( i != 0 ) {
								// finish scroll
								$slidesMedia.eq(i).css('clip', 'rect(0 ' + Math.ceil($(window).width() / 2) + 'px ' + $(window).height() + 'px 0)');
						...
						if ( $slides.eq(i-1).length >= 0 && $(window).scrollTop() + $(window).height() < $slides.eq(i).offset().top ) {
						
							// finish scroll

							var stupid = $slidesMedia.eq(i).hasClass('obs') ? 1 : 0;

							$slidesMedia.eq(i).css('clip', 'rect(0 ' + Math.ceil($(window).width() / 2) + 'px ' + stupid + 'px 0)');
							if ( slidesBlackArray[j] ) {
					...
					if ( ! $slider.hasClass('back-to-normal') ) {
						...
						var scrollValue = Math.ceil($(window).scrollTop() - $(window).height() * j );

						var stupid = $slidesMedia.eq(i).hasClass('obs') ? 1 : 0;
							
						$slidesMedia.eq(i).css('clip', 'rect(0 ' + Math.ceil($(window).width() / 2) + 'px ' + Math.max(stupid, scrollValue) + 'px 0)');

						if ( slidesBlackArray[j] ) {
							slidesBlackArray[j].css('opacity', ( Math.ceil(scrollValue * 50 / $(window).height()) ) / 100 );
						}

						var scrollFactor = Math.round($(window).height() / 6);

						$slides.eq(j).find('.caption').css('transform', 'translateY(' + ( 0 - Math.ceil(scrollValue * ( scrollFactor*1 ) / $(window).height() ) ) + 'px)');
						$slides.eq(j).find('.title').css('transform', 'translateY(' + ( 0 - Math.ceil(scrollValue * ( scrollFactor*0.75 ) / $(window).height() ) ) + 'px)');
						$slides.eq(j).find('.subtitle').css('transform', 'translateY(' + ( 0 - Math.ceil(scrollValue * ( scrollFactor*0.5 ) / $(window).height() ) ) + 'px)');
						$slides.eq(j).find('.button').css('transform', 'translateY(' + ( 0 - Math.ceil(scrollValue * ( scrollFactor*0.25 ) / $(window).height() ) ) + 'px)');

						$slides.eq(i).find('.caption').css('transform', 'translateY(' + ( Math.ceil(scrollValue * ( scrollFactor * 1) / $(window).height()) - ( scrollFactor * 1) ) + 'px)');
						$slides.eq(i).find('.title').css('transform', 'translateY(' + ( Math.ceil(scrollValue * ( scrollFactor * 0.75) / $(window).height()) - ( scrollFactor * 0.75) ) + 'px)');
						$slides.eq(i).find('.subtitle').css('transform', 'translateY(' + ( Math.ceil(scrollValue * ( scrollFactor * 0.5) / $(window).height()) - ( scrollFactor * 0.5) ) + 'px)');
						$slides.eq(i).find('.button').css('transform', 'translateY(' + ( Math.ceil(scrollValue * ( scrollFactor * 0.25) / $(window).height()) - ( scrollFactor * 0.25) ) + 'px)');
